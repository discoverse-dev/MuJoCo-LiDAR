...
def pyramid_stairs_terrain(terrain, step_width, step_height, platform_size=1., base_platform=0.):
    """
    Generate stairs
    Parameters:
...
def main():
    ...
    parser.add_argument("--resolution", type=int, default=500, help="Image resolution (width/height)")
    parser.add_argument("--size", type=float, default=12.0, help="Physical size of terrain in meters")
    ...
    parser.add_argument("--base_platform", type=float, default=1.0, help="Size of base platform (m)")
    parser.add_argument("--output_dir", type=str, default="output/generated_terrain", help="Output directory")
    ...
    print(f"Sub-terrain size: {sub_w}x{sub_l} pixels ({sub_w*horizontal_scale:.2f}m x {sub_l*horizontal_scale:.2f}m)")
    ...
    for i in range(n):
        for j in range(n):
            ...
            pyramid_stairs_terrain(sub, step_width_m, h, platform_size=args.platform_size, base_platform=effective_base_platform)
            ...
            if r_end > width: r_end = width
            if c_end > length: c_end = length
